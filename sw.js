 ...
 */
const CACHE_VERSION = 'eotr2026-v1.59.0';
...
async function networkFirst(request, fallbackUrl) {
  ...
  try {
    ...
    // The cached copy of THIS request, or an explicit fallback (index.html for
    // navigations). Data files fall back to their own precached copy so offline
    // still shows the last-fetched data, never the HTML shell.
    const own = await caches.match(request);
    if (own) return own;
    const cached = await caches.match(fallbackUrl || './index.html');
...
self.addEventListener('fetch', (event) => {
  ...
  if (url.origin === self.location.origin) {
    // Festival data must never go stale: always try the network first, falling
    // back to the cached copy offline. This way an updated data file is served
    // even if someone forgets to bump CACHE_VERSION.
